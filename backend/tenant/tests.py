...
class TestAPIJWT(BaseTestCase):
    ...
    def test_user_list(self):
        ...
        self.assertEqual(url, '/api/get_user_list')
    ...
    def test_user_profile(self):
        ...
        self.assertEqual(url, '/api/profile_detalis')
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_user_profile_update(self):
        ...
        self.assertEqual(url, '/api/update_user_info')
...
class TestApplicationSetting(BaseTestCase):
    def test_email_notification(self):
        ...
        self.assertEqual(url, '/api/haveEmailNotification')
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)

    def test_two_factor_auth(self):
        url = reverse('check_have_two_factor_auth')
        self.assertEqual(url, '/api/havetwoFactorAuth')
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_update_email_notification(self):
        ...
        self.assertEqual(url, '/api/updateEmailNotification')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    
    def test_update_two_factor_auth(self):
        url = reverse('update_two_factor_auth')
        resp = self.client.put(url)
        self.assertEqual(url, '/api/update_two_factor_auth')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
