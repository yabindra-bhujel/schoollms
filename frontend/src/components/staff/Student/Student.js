...
const AdminStudent = () => {
	...
	const handleConfirmDelete = async () => {
		try {
			...
			setStudentList((prevList) => prevList.filter((student) => student.student_id !== studentToDelete));
	...
	return (
		<AdminLayout>
			<div className='admin-student'>
				<div className='admin-student-header'>
					...
					<div className='student-add-btn'>
						<input type='file' style={{ display: "none" }} accept='.csv' onChange={handleFileSelect} ref={fileInputRef} />
						<Button variant='contained' color='primary' onClick={() => fileInputRef.current.click()} disabled={uploadingFile}>
						...
						<TableContainer>
							<Table>
								...
								<TableBody className='admin-student-table-body'>
									{studentList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((student) => (
										<TableRow key={student.student_id}>
											<TableCell>
												<Avatar alt={student.first_name} src={student.image} sx={{ width: 40, height: 40 }} />
											</TableCell>
											...
											<TableCell>
												<div className='action-btn'>
													<Button onClick={() => handleStudentDelete(student.student_id)} variant='outlined' color='secondary'>
														Delete
													</Button>
												</div>
											</TableCell>
			...
			<Dialog open={openDialog} onClose={handleCloseDialog} aria-labelledby='alert-dialog-title' aria-describedby='alert-dialog-description'>
				...
				<DialogContent>
					<DialogContentText id='alert-dialog-description'>Are you sure you want to permanently delete this student? This action cannot be undone.</DialogContentText>
				</DialogContent>
				<DialogActions>
